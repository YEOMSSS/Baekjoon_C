...
// 1. 시작 정점 v 방문
// 2. v 인접 정점 중 방문하지 않은 정점을 오름차순 방문하며 큐에 넣음
// 3. 방문하지 않은 인접한 정점이 없으면, 큐에서 꺼낸 정점을 v로 설정하고 2 반복
// 4. 큐가 공백이 될 때까지 2~3을 반복
void BFS_adjList(graphType *g, int v)
{
    ...
    // 4. 큐가 공백인 아닌 동안 너비 우선 탐색 수행
    while (!isEmpty(Q))
    {
        // 3. deQueue 정점꺼내기
        v = deQueue(Q);
        // 2. 현재 정점 w를 방문하지 않은 경우
        for (w = g->adjList_H[v]; w; w = w->link) // 인접 방문이 있는 동안 수행
