...
// 그냥 호출될때마다 0부터 오름차순으로 하나씩 해금하는 느낌이다. v가 의미 없는 함수.
...
void insertEdge(graphType *g, int u, int v)
{
    ...
    node->vertex = v;
    // 맨 뒤에 안붙이고 그냥 헤드 뒤에 바로 붙여버리면 훨씬 편하다.
    node->link = g->adjList_H[u]; // 삽입 간선에 대한 노드를 리스트의 첫 번째 노드로 연결(현재 헤드포인터)
    g->adjList_H[u] = node;       // 헤드포인터를 노드로 만들어버린다.
    // 오름차순으로 구현하고싶다면 내림차순으로 삽입해야하겠다.
}
...
int main(void)
{
    ...
    // 그래프 G1
    for (i = 0; i < 4; i++)  // 4회 호출해서 0~3정점 사용가능하게 만들기
        insertVertex(G1, i); // G1의 정점 0~3 삽입

    // 간선을 내림차순으로 삽입해야 출력이 오름차순으로 된다.
    insertEdge(G1, 0, 3);
    insertEdge(G1, 0, 1);
