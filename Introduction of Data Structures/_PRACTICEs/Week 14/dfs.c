...
typedef struct graphType
{
    ...
    int visited[MAX_VERTEX];          // 정점에 대한 방문 표시를 위한 배열 << visited가 추가됨.
} graphType;
...
// 1. 시작 정점 v 방문
// 2. v 인접 정점 중
// 2-1. 방문하지 않은 정점 w가 있으면 v를 스택에 push하고 w를 방문, 그리고 w를 v로 하여 2를 반복
// 2-2. 방문하지 않은 정점이 없으면 스택을 pop하여 가장 마지막에 방문한 정점을 v로 한 뒤 2를 반복
// 3. 스택이 공백이 될 때까지 2를 반복

void DFS_adjList(graphType *g, int v)
{
    // w는 현재 탐색하는 정점, v는 시작 정점
    graphNode *w;
    // 1. 시작 정점 v에 대해 처리
    top = NULL;           // 스택의 top 설정
